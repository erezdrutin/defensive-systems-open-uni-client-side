...
std::string RSAPrivateWrapper::getPublicKey() const
{
    ...
    std::string derKey, pemKey;
    CryptoPP::StringSink ss(derKey);
    publicKey.Save(ss);

    CryptoPP::Base64Encoder encoder;
    encoder.Put((const CryptoPP::byte *)derKey.data(), derKey.size());
    encoder.MessageEnd();
    CryptoPP::word64 size = encoder.MaxRetrievable();
    if(size)
    {
        pemKey.resize(size);
        encoder.Get((CryptoPP::byte*)pemKey.data(), pemKey.size());
    }
    pemKey.insert(0, "-----BEGIN PUBLIC KEY-----\n");
    pemKey.append("\n-----END PUBLIC KEY-----\n");
    return pemKey;
}
